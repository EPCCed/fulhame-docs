...
.. note:: There are a number of different queues on Fulhame. In general, you should not specify a queue and the submission system will select the correct one for your job.

If you have any questions on how to run jobs on Fulhame do not hesitate
to contact the `EPCC Helpdesk <mailto:support@epcc.ed.ac.uk>`_.
...
This will submit your job script "job\_script.pbs" to the job-queue.
...
Use this command to delete a job from Fulhame's job queue. For example:
...
Queues on Fulhame are designed to enable users to use the system flexibly while 
...
::
    ...
    [user@fulhame-login1 ~]$ qsub submit.pbs 
...
for different kinds of parallel jobs on Fulhame.
...
-  The number of nodes and cores per node you require via the
   ``-l select=[Nodes]:ncpus=64`` option. Each node has 64 physical
   cores (2x 32-core sockets). For example, to select 4 nodes
   (256 physical cores in total) you would use
   ``-l select=4:ncpus=64``. **We strongly recommend that all parallel
...
this mode of use is strongly recommended for all parallel jobs on Fulhame.
...
::
   ...
   [auser@fulhame-login1 ~]$ module show vasp
   -------------------------------------------------------------------
...
The HPE MPT parallel launcher on Fulhame is ``mpiexec_mpt``.
...
The most important ``mpiexec_mpt`` flags are:
 ...
 ``-ppn [parallel processes per node]``
    ...
    node. There is a choice of 1-36 for physical cores on Fulhame compute
...
takes the number of threads as the option ``-nt``:
 ...
 ``-nt [threads per parallel process]``
    ...
    your shared memory programming.) The default on Fulhame is 1.
...
The Intel MPI parallel job launcher on Fulhame is ``mpirun``.
...
Serial jobs are setup in a similar way to parallel jobs on Fulhame. The
only changes are:
...
best ways to achieve this on Fulhame is to use interactive jobs.
...
Resource reservations are available on Fulhame. These allow users to reserve
...
You request a reservation on Fulhame using PBS from the command line. Before 
...
::
   ...
   [auser@fulhame-login0 ~]$ pbs_rdel R122605
